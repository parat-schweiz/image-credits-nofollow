...
class BetterImageCreditsAdmin {
	...
	function add_settings() {
		...
		$this->add_settings_field('image-credits-nofollow_pages', __('Pages', 'image-credits-nofollow'), 'add_settings_field_pages');
	...
	function add_settings_field_pages() { ?>
		<p>
			<label>
				<input type="checkbox" name="image-credits-nofollow_pages" value="pages" <?php checked(IMAGE_CREDIT_PAGES, 'pages'); ?>>
				<?php _e('Show image credits on pages', 'image-credits-nofollow'); ?>
			</label>
		</p>
		<p><em><?php _e('Choose if you want to display the image credits also on pages', 'image-credits-nofollow'); ?></em></p>
	<?php }	
	...
	function add_settings_field_sep() { ?>
		...
		<p><em><?php _e('HTML to separate the credits (enter leading and trailing spaces using HTML entities). Default:', 'image-credits-nofollow'); echo(' <code>' . htmlspecialchars(',&#32;') . '</code>') ?></em></p><?php
	}
	...
	function add_settings_field_before() { ?>
		...
		<p><em><?php _e('HTML to output before the credits (enter leading and trailing spaces using HTML entities).  Default:', 'image-credits-nofollow');  echo(' <code>' . htmlspecialchars('<p class="image-credits">') . __('Image Credits', 'image-credits-nofollow') . htmlspecialchars(':&#32;') . '</code>') ?></em></p><?php
	}
	...
	function add_settings_field_after() { ?>
		...
		<p><em><?php _e('HTML to output after the credits (enter leading and trailing spaces using HTML entities). Default:', 'image-credits-nofollow'); echo(' <code>' . htmlspecialchars('</p>') . '</code>')  ?></em></p><?php
	}
